...
are stored and written verbatim. On Windows the effective path limit for tgxiv
is 260 UTF-16 characters (Go binaries embed no long-path-aware manifest), so a
root prefix that is too deep surfaces as ordinary download failures whose
error text names the path length.
